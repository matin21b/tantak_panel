...
export const actions = {
  ...
  getRoleServer({ commit }) {
    return new Promise((res, rej) => {
      this.$reqApi(`/role`, { row_number: 2000 })
        .then(async (response) => {
          let roles = response.model.data.map((x) => ({
            ...x,
            text: x.name,
            value: x.id,
          }));
  ...
  getRoleByActionServer({ commit }, body) {
    return new Promise((res, rej) => {
      this.$reqApi(`/role/get-by-action`, body)
        .then(async (response) => {
          let roles = response.model.map((x) => ({
            text: x.name,
            value: x.id,
          }));
    ...
    this.$reqApi("/country-division", { row_number: 3000 })
      .then((res) => {
        let data = {
          ...
          rural_district: res.model.data.filter(
            (x) => x.level == "rural_district"
          ),
  ...
  setBrand({ commit }) {
    this.$reqApi("/brand", { row_number: 40009 })
      .then((res) => {
        let items = [];
