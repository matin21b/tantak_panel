<template>
    <div class="d-flex  my-4 mt-7  ml-2 mr-4">
        <div :class="is_mobile? 'd-flex align-center flex-column mr-12':'d-flex align-center' ">
            <span>
                        انتخاب بازه زمانی از
                    </span>
            ...
            <amp-button class="mt-1" text="جستجو بازه زمانی" @click="selecDate()" :loading="loading" :disabled="!Boolean(date_score_to)" />
        </div>
    </div>
</template>
...
export default {
    ...
    beforeMount() {
        ...
        this.date_score_of = jmoment(this.$moment(this.now).subtract(30, 'days')).format('YYYY-MM-DD')
