        ...
        <v-card-text>
          <v-window v-model="step">
                    ...
                    <v-expansion-panel>
                      ...
                      <v-expansion-panel-content>
                        <v-form v-model="valid" @submit.prevent="submit()" class="mt-4 pa-5" v-if="!loading">
                          <SelectVariation @validVariations="continue_form = $event" v-if="!update"
                            @productId="form.product_var_id = $event" :productInfo="product" :response="response"
            ...
            <v-window-item :value="2">
              <History :branchId="$store.state.auth.user.sale_agency_id" v-if="show_history" :productVarId="product_var_id"
