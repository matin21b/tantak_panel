<template>
  <v-form v-model="valid" @submit.prevent="submit()" :disabled="loader" class="rounded-0 pa-2 d-flex flex-column">
    ...
    <v-row class="ma-1 d-flex justify-center">
      ...
      <amp-button :text="modelId ? 'ویرایش': 'افزودن'" type="submit" icon="done" class="ma-1" :loading="loader"></amp-button>
    </v-row>
  </v-form>
</template>
...
export default {
  props: {
    ...
    updateUrl: { default: 'action-category/update' },
    createUrl: {default : 'action-category/insert'},
    showUrl: { default: 'action-category/show' },
  },
  data() {
    return {
      valid: true,
